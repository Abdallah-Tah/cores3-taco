...
esp_err_t Ota::CheckVersion() {
    ...
    // A private XiaoZhi server can be selected at build time without needing
    // a separate OTA/activation endpoint to return the WebSocket settings.
    if (strlen(CONFIG_CUSTOM_WEBSOCKET_URL) > 0) {
        Settings websocket_settings("websocket", true);
        websocket_settings.SetString("url", CONFIG_CUSTOM_WEBSOCKET_URL);
        ...
        ESP_LOGI(TAG, "Using direct WebSocket server: %s", CONFIG_CUSTOM_WEBSOCKET_URL);
        return ESP_OK;
    }
